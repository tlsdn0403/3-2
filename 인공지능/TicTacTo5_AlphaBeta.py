...
# 알파베타 가지치기 미니맥스 알고리즘
def minimax_alphabeta(board, depth, maxPlayer, alpha, beta):
    pos = -1
    ...
    if maxPlayer:
        ...
        for p in empty_cells(board):
            board[p] = 'X'
            _, score = minimax_alphabeta(board, depth - 1, False, alpha, beta)
            board[p] = ' '
            if score > value:
                value = score
                pos = p
        ...
        for p in empty_cells(board):
            board[p] = 'O'
            _, score = minimax_alphabeta(board, depth - 1, True, alpha, beta)
            board[p] = ' '
            if score < value:
                value = score
                pos = p
...
while True:
    ...
    i, v = minimax_alphabeta(game_board, 5, player == 'X', -10000, 10000)
    move(i, player)
